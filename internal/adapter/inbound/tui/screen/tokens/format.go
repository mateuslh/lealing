...
func providerTone(th *theme.Theme, provider string) lipgloss.TerminalColor {
	switch provider {
	case "Claude Code":
		return th.Warning
	case "Codex":
...
func modelTone(th *theme.Theme, model string) lipgloss.TerminalColor {
	switch {
...
func quotaTone(th *theme.Theme, usedPercent float64) lipgloss.TerminalColor {
	switch {
	case usedPercent >= 90:
		return th.Danger
	case usedPercent >= 70:
